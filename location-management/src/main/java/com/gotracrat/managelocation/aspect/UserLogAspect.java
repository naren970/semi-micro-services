package com.gotracrat.managelocation.aspect;
...
public class UserLogAspect {
    ...
    @Before(value = "execution(* com.gotracrat.managelocation.service.*.*(..))")
    public void beforeAdvice(JoinPoint joinPoint) {
        System.out.println("Before method:" + joinPoint.getSignature());
        System.out.println("Successfully Executing Location Calls - ");
    }
    
    @After(value = "execution(* com.gotracrat.managelocation.service.*.*(..))")
    public void afterAdvice(JoinPoint joinPoint) {
        UserLog userLogAction=null;
        Object[] argsArr = joinPoint.getArgs();
        for (Object signatureArg: argsArr) {
            String className = signatureArg.getClass().getName();
            if(className.equalsIgnoreCase("com.gotracrat.managelocation.entity.UserLog")){
